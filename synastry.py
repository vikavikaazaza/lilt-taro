...
SYNASTRY_ORBS = {
    'Солнце': 8.0, 'Луна': 8.0, 'Меркурий': 6.0, 'Венера': 6.0,
    'Марс': 6.0, 'Юпитер': 5.0, 'Сатурн': 5.0, 'Уран': 4.0,
    'Нептун': 4.0, 'Плутон': 4.0,
}
ANGLE_ORB = 5.0
...
def _natal_chart(birth_date: date, birth_time: time | None, lat: float, lon: float, tz_name: str, city: str) -> dict[str, Any]:
    ...
    if time_known:
        try:
            ...
            mc = 0.0
        angles = {'ASC': asc, 'MC': mc, 'DSC': normalize_deg(asc + 180.0), 'IC': normalize_deg(mc + 180.0)}
    return {
        'birth_date': birth_date.isoformat(),
        'birth_time': birth_time.strftime('%H:%M') if birth_time else None,
        'time_known': time_known, 'city': city, 'timezone': tz_name,
        'latitude': float(lat), 'longitude': float(lon), 'jd': jd,
        'planets': planets, 'houses': cusps, 'angles': angles,
...
def calculate_synastry(person1: dict[str, Any], person2: dict[str, Any], name1: str = '', name2: str = '') -> dict[str, Any]:
    p1 = _natal_chart(person1['birth_date'], person1.get('birth_time'), person1['lat'], person1['lon'], person1['timezone'], person1['city'])
    p2 = _natal_chart(person2['birth_date'], person2.get('birth_time'), person2['lat'], person2['lon'], person2['timezone'], person2['city'])
    ...
    for n1, _ in PLANETS:
        for n2, _ in PLANETS:
            ...
            aspects.append({
                'person1_planet': n1, 'person2_planet': n2, 'aspect': aspect,
                'orb': round(orb, 2), 'orb_text': format_degree(orb),
                'person1_sign': p1['planets'][n1]['sign'], 'person1_position': p1['planets'][n1]['position'],
                'person2_sign': p2['planets'][n2]['sign'], 'person2_position': p2['planets'][n2]['position'],
    ...
    if p2['time_known']:
        for n1, _ in PLANETS:
            for angle in ANGLES:
                ...
                if lon2 is None: continue
                best = _best_aspect(p1['planets'][n1]['longitude'], lon2)
                if best and best[1] <= ANGLE_ORB:
                    aspect, orb, _ = best
                    angle_aspects.append({'from_person': 1, 'planet': n1, 'to_person': 2, 'point': angle, 'aspect': aspect, 'orb': round(orb, 2), 'orb_text': format_degree(orb)})
    if p1['time_known']:
        for n2, _ in PLANETS:
            for angle in ANGLES:
                lon1 = p1['angles'].get(angle)
                if lon1 is None: continue
                best = _best_aspect(p2['planets'][n2]['longitude'], lon1)
                if best and best[1] <= ANGLE_ORB:
                    aspect, orb, _ = best
                    angle_aspects.append({'from_person': 2, 'planet': n2, 'to_person': 1, 'point': angle, 'aspect': aspect, 'orb': round(orb, 2), 'orb_text': format_degree(orb)})
    ...
    if p2['time_known']:
        for n1, _ in PLANETS:
            ...
            if house: overlays_1_in_2.append({'planet': n1, 'house': house})

    overlays_2_in_1 = []
    if p1['time_known']:
        for n2, _ in PLANETS:
            house = _house_for_longitude(p2['planets'][n2]['longitude'], p1['houses'])
            if house: overlays_2_in_1.append({'planet': n2, 'house': house})

    return {
        'name1': name1.strip(), 'name2': name2.strip(),
        'person1': p1, 'person2': p2,
        'aspects': aspects, 'angle_aspects': angle_aspects,
        'overlays_1_in_2': overlays_1_in_2, 'overlays_2_in_1': overlays_2_in_1,
        'aspect_count': len(aspects), 'angle_aspect_count': len(angle_aspects),
...
def calculation_for_ai(calc: dict[str, Any]) -> str:
    lines = [
        ...
        f"Часовой пояс: {calc['person1']['timezone']}", 'Планеты:',
    ]
    for name, _ in PLANETS: lines.append(_planet_line(name, calc['person1']['planets'][name]))
    if calc['person1']['time_known']:
        lines.append(f"Асцендент: {calc['person1']['ascendant_sign']}")
        lines.append(f"MC: {calc['person1']['mc_sign']}")
    else: lines.append('Время рождения неизвестно: дома и углы не используются.')
    ...
    lines += ['', 'ЧЕЛОВЕК 2:',
              ...
              f"Часовой пояс: {calc['person2']['timezone']}", 'Планеты:']
    for name, _ in PLANETS: lines.append(_planet_line(name, calc['person2']['planets'][name]))
    ...
    if calc['aspects']:
        for a in calc['aspects']:
            lines.append(f"- {a['person1_planet']} ({a['person1_sign']} {a['person1_position']}) {a['aspect']} {a['person2_planet']} ({a['person2_sign']} {a['person2_position']}) (орб {a['orb_text']}, {a['relationship_weight']})")
    else:
